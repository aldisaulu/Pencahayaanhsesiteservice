...
async function exportToPdf() {
    ...
    try {
        ...
        // Validasi lebih ketat biar pesan error jelas
        if (!jspdfLibrary || !jspdfLibrary.jsPDF) {
            console.error('jsPDF library load gagal. Detil:', {
                windowJspdf: !!(window.jspdf && window.jspdf.jsPDF),
                keys: window.jspdf ? Object.keys(window.jspdf) : null
            });
            alert('Library PDF belum tersedia. Pastikan file libs/jspdf.umd.min.js tidak error (404) dan/atau internet aktif untuk fallback CDN.');
            return;
        }
